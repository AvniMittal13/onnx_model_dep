...
class BasicNCA3D(nn.Module):
    def __init__(self, channel_n, fire_rate, hidden_size=128, input_channels=1, init_method="standard", kernel_size=7, groups=False):
        r"""Init function
            #Args:
...
class M3DNCA(nn.Module):
    ...
    """
    def __init__(self, channel_n, fire_rate, steps=64, hidden_size=128, input_channels=1, output_channels=1, scale_factor=4, levels=2, kernel_size=7):
        r"""Init function
            #Args:
                channel_n: number of channels per cell
                fire_rate: random activation of each cell
        ...
        for i in range(self.levels):
            ...
            # ks = 3 + 4*i
            self.model.append(BasicNCA3D(channel_n=channel_n, fire_rate=fire_rate, hidden_size=hidden_size, input_channels=input_channels, kernel_size=ks))

    def make_seed(self, x):
        seed = torch.zeros((x.shape[0], x.shape[1], x.shape[2], x.shape[3], self.channel_n), dtype=torch.float32)
        seed[..., 0:x.shape[-1]] = x
        return seed
    ...
    def forward_train(self, x: torch.Tensor, y: torch.Tensor):
        ...
        for m in range(self.levels):
            ...
            if m == self.levels-1:
                ...
                inputs_loc = torch.zeros((inputs_loc_temp.shape[0], size[0], size[1], size[2] , inputs_loc_temp.shape[4]))
                targets_loc = torch.zeros((targets_loc_temp.shape[0], size[0], size[1], size[2] , targets_loc_temp.shape[4]))
                full_res_new = torch.zeros((full_res.shape[0], int(full_res.shape[1]/self.scale_factor), int(full_res.shape[2]/self.scale_factor), int(full_res.shape[3]/self.scale_factor), full_res.shape[4]))
                full_res_gt_new = torch.zeros((full_res.shape[0], int(full_res.shape[1]/self.scale_factor), int(full_res.shape[2]/self.scale_factor), int(full_res.shape[3]/self.scale_factor), full_res_gt.shape[4]))
